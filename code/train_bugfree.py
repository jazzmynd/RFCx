...
# if no validation set, get it

def get_aug_type(data_dir):
    aug_type = ""
    for c in os.listdir(data_dir+"/p/"):
        for f in os.listdir(data_dir+"/p/"+c):
            if "aug" in f:
                aug_type = f.split("aug")[0].split("_")[-2]
                return aug_type
...
if not os.path.exists(data_dir + "/train"):
    ...
    os.mkdir(data_dir + "/train/p")
    for c in class_list:
        os.mkdir(data_dir + "/train/p/"+c)
    
    os.mkdir(data_dir + "/train/n")
    for c in class_list:
        os.mkdir(data_dir + "/train/n/"+c)
    
    os.mkdir(data_dir + "/val")
    os.mkdir(data_dir + "/val/p")
    for c in class_list:
        os.mkdir(data_dir + "/val/p/"+c)
    os.mkdir(data_dir + "/val/n")
    for c in class_list:
        os.mkdir(data_dir + "/val/n/"+c)
        
    # copy file
    aug_type = get_aug_type(data_dir)
    p_train = []
    with open("./train_val/pos_train_file.txt", "r") as f:
        for line in f:
            p_train.append(line.strip())
#     print(p_train)
    for f in p_train:
        shutil.copyfile(data_dir+"/p/"+f, data_dir + "/train/p/"+ f)
        if aug_type != "":
            aug_file = f[0:-4] + "_"+ aug_type+ "_aug.npy" 
            shutil.copyfile(data_dir+"/p/"+aug_file, data_dir + "/train/p/"+ aug_file)
    
    p_val = []
    with open("./train_val/pos_val_file.txt", "r") as f:
        for line in f:
            p_val.append(line.strip())
    for f in p_val:
        shutil.copyfile(data_dir+"/p/"+f, data_dir + "/val/p/"+ f)
        if aug_type != "":
            aug_file = f[0:-4] + "_"+ aug_type+ "_aug.npy" 
            shutil.copyfile(data_dir+"/p/"+aug_file, data_dir + "/val/p/"+ aug_file)
    
    
    n_train = []
    with open("./train_val/neg_train_file.txt", "r") as f:
        for line in f:
            n_train.append(line.strip())
    for f in n_train:
        shutil.copyfile(data_dir+"/n/"+f, data_dir + "/train/n/"+ f)
    
    n_val = []
    with open("./train_val/neg_val_file.txt", "r") as f:
        for line in f:
            n_val.append(line.strip())
    for f in n_val:
        shutil.copyfile(data_dir+"/n/"+f, data_dir + "/val/n/"+ f)
...
files_train_p, _, labels = get_files_and_labels(data_dir+'/train/p/', train_split=train_split, random_state=42, classes=class_list)

# generate negative train file paths
files_train_n, _, labels_n = get_files_and_labels(data_dir+'/train/n/', train_split=train_split, random_state=42, classes=class_list) 

# generate positive val file paths
files_val_p, _, labels = get_files_and_labels(data_dir+'/val/p/', train_split=train_split, random_state=42, classes=class_list)

# generate negative val file paths
files_val_n, _, labels_n = get_files_and_labels(data_dir+'/val/n/', train_split=train_split, random_state=42, classes=class_list) 
